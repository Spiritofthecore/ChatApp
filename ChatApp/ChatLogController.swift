...
class ChatLogController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let text = messages?[indexPath.row].text, let message = messages?[indexPath.item], let profileImageName =  messages?[indexPath.item].friend?.profileImageName {
            ...
            let estimatedFrame = NSString(string: text).boundingRect(with: sizeToFit, options: options, attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 16)], context: nil)
            if !message.isSender {
